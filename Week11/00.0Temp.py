...
def main(times, a_cat, a_fox, count, data_list):
    ...
    for _ in range(times):
        ...
        temp = [spl_d[1], spl_d[3]] if len(spl_d) > len(spl_s) else [
            spl_s[1], spl_s[3]]
        data_list.append(
            {'N': temp[0], 'T': temp[1][:3], 'O': int(temp[1][3:])})
    for i in data_list:
        a_cat = False if i['N'] == 'Garfield' or (
            i['T'] == 'Cat' and i['O'] == 1) else a_cat
        a_fox = False if i['N'] == 'Fubuki' or (
            i['T'] == 'Fox' and i['O'] == 1) else a_fox
